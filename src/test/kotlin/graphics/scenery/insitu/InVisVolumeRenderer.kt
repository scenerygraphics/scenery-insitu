...
class InVisVolumeRenderer: SceneryBase("InVisVolumeRenderer") {
    ...
    private external fun distributeVDIs(subVDI: ByteBuffer, sizePerProcess: Int, commSize: Int)
    private external fun gatherCompositedVDIs(compositedVDI: ByteBuffer, root: Int, subVDILen: Int, myRank: Int, commSize: Int)
...
//        }
    ...
    private fun manageVDIGeneration() {
        while(true) {
            ...
            var str1 = "This is the 0th part of the VDI generated by process 0"
            var tempBuffer: ByteBuffer = ByteBuffer.allocateDirect(2 * str1.length * commSize)

            for (i in 0 until commSize) {
                str1 = "This is the ${i}th part of the VDI generated by process $rank"
                tempBuffer = tempBuffer.put(str1.toByteArray(Charsets.UTF_16BE))
            }

            tempBuffer.position(0)

//            logger.info("Calling the distributeVDIs function. CommSize is $commSize, length is ${str1.length} and the buffer is ${tempBuffer.asCharBuffer()}")
            distributeVDIs(tempBuffer, str1.length * 2, commSize)

            //fetch the composited VDI
            val compositedVDILen = "This is process 0 sending it's composited VDI!".length

            val temp = ByteBuffer.allocateDirect(compositedVDILen)
            val str2 = "This is process $rank sending it's composited VDI!" //This is the composited VDI which needs to be tiled
            temp.put(str2.toByteArray(Charsets.UTF_8))
            temp.order(ByteOrder.nativeOrder())
            logger.info("Calling gather function")
            gatherCompositedVDIs(temp,0, compositedVDILen, rank, commSize)
            Thread.sleep(1000)
        }
    }
...
//                    volumesFromThisPartner[grid]?.purgeFirst(0, 1)
    ...
    @Suppress("unused")
    fun compositeVDIs(VDISet: ByteBuffer, sizePerProcess: Int) {
